...
classStr)return true;return false},callDelegate:function(methodName,params){if(this.delegate!==null&&typeof this.delegate[methodName]!=="undefined")this.delegate[methodName].apply(this.delegate,params)},clone:function(){return Object.create(this)}});Class("Exception",{message:null,Exception:function(message){this.message=message},toString:function(){return this._class+": "+this.message+" "}});Class("MissingAnnotationException::Exception",{MissingAnnotationException:function(annotationName){this.message=
annotationName+" annotation is missing"}});Class("InvalidParameterException::Exception",{InvalidParameterException:function(expectedType,parameterIndex){this.message="parameter "+parameterIndex+" should be "+expectedType}});Class("UnsupportedOperationException::Exception",{UnsupportedOperationException:function(methodName){this.message=methodName+" method is not implemented yet"}});Class("DeprecatedException::Exception",{DeprecatedException:function(oldMethod,newMethod){this.message="Method "+oldMethod+
" is deprecated, please use "+newMethod+" instead"}});Class("View",{element:null,controller:null,model:null,elementType:"div",elementClass:"view",View:function(){this.element=document.createElement(this.elementType);this.element.className=this.elementClass},addClasses:function(){this.element.className="";for(var i=0;i<arguments.length;i++)this.element.className+=arguments[i]+" "},getViews:function(selector){return this.element.querySelector(selector)},addViews:function(template){var ph=Defaults.viewPlaceholder;
for(var k=1;k<arguments.length;k++)if(arguments[k]!==null&&typeof(arguments[k]!=="undefined"))if(typeof arguments[k]==="string"||typeof arguments[k]==="boolean"||typeof arguments[k]==="number")template=template.replace(ph,arguments[k]);else if(arguments[k].isExtends("View")||arguments[k]._class==="View")template=template.replace(ph,"<div class='view-placeholder'></div>");var elementHandler=document.createElement("div");elementHandler.innerHTML=template;for(var i=1;i<arguments.length;i++)if(arguments[i]!==
null&&typeof arguments[i]!=="undefined")if(arguments[i].isExtends&&(arguments[i].isExtends("View")||arguments[i]._class==="View")){var view=arguments[i];if(typeof this.delegate!=="undefined"&&view.delegate===null&&this.delegate!==null&&this._class!=="Viewport")view.setDelegate(this.delegate);if(typeof this.model!=="undefined"&&view.model===null&&this.model!==null&&this._class!=="Viewport")view.setModel(this.model);view.render();var replacableNode=elementHandler.querySelector(".view-placeholder");
var replacableParentNode=replacableNode.parentNode;replacableParentNode.appendChild(view.element);replacableParentNode.removeChild(replacableNode)}if(elementHandler.hasChildNodes())while(elementHandler.childNodes.length>=1)this.element.appendChild(elementHandler.firstChild)},clearViews:function(){var cell=this.element;if(cell.hasChildNodes())while(cell.childNodes.length>=1)cell.removeChild(cell.firstChild)},removeView:function(){this.element.parentNode.removeChild(this.element)},addEvent:function(selector,
eventType,eventName,preventDefault){var _self=this;preventDefault=typeof preventDefault==="undefined"?false:preventDefault;var eventCallback=function(){return function(evt){if(preventDefault)evt.preventDefault();_self.handleEvents.apply(_self,[evt,eventName])}}.apply(_self);var elements=this.element.querySelectorAll(selector);if(elements.length===0)elements=[this.element];for(var i=0;i<elements.length;i++)elements[i][eventType]=eventCallback},handleEvents:function(eventObject,eventName){Throw(new UnsupportedOperationException("handleEvents"))},
render:function(){return this.element}});Class("Viewport::View",{width:typeof window!=="undefined"?window.innerWidth:0,height:typeof window!=="undefined"?window.innerHeight:0,init:function(delegate){this.delegate=delegate;this.start()},start:function(){var _self=this;if(document.addEventListener){window.addEventListener("load",function(){_self.element=document.body;_self.callDelegate("didBecomeActive",[_self])},false);window.addEventListener("beforeunload",function(){_self.callDelegate("willResignActive",
[_self])},false);window.addEventListener("unload",function(){_self.callDelegate("willTerminate",[_self])},false)}else{window.attachEvent("onload",function(){_self.callDelegate("didBecomeActive",[_self])});window.attachEvent("onbeforeunload",function(){_self.callDelegate("willResignActive",[_self])});window.attachEvent("onunload",function(){_self.callDelegate("willTerminate",[_self])})}}});_globalsObject.Class=Class;_globalsObject.LocalClass=LocalClass;_globalsObject.Throw=Throw;_globalsObject.Catch=
Catch})();