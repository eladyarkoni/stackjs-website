/**
*
*	StackJS Framework Version 0.1.0
*	Author: Elad Yarkoni
*
*
*/
	...
	*********************************************************/
	Class('View', {
		...
		addViews: function(template) {
			var viewPlaceholder = Defaults.viewPlaceholder;
			var container = document.createElement('div');
			container.innerHTML = template;
			for (var i = 1; i < arguments.length; i++) {
				if (arguments[i] === null) {
					// do nothing
				} else if ((typeof(arguments[i]) === 'string') || (typeof(arguments[i]) === 'boolean') || (typeof(arguments[i]) === 'number')) {
					// Add String
					container.innerHTML = container.innerHTML.replace(viewPlaceholder, arguments[i]);
				} else if ((arguments[i].isExtends('View')) || (arguments[i]._class === "View")) {
					// Add View
					container.innerHTML = container.innerHTML.replace(viewPlaceholder, "<div class='_replacable'></div>");
					var replacableDiv = container.querySelector('._replacable');
					var replacableDivParent = replacableDiv.parentNode;
					replacableDivParent.removeChild(replacableDiv);
					if ((typeof(this.delegate) !== 'undefined') && (arguments[i].delegate === null) && (this.delegate !== null) && (this._class !== 'Viewport')) {
						arguments[i].setDelegate(this.delegate);
					}
					if ((typeof(this.model) !== 'undefined') && (arguments[i].model === null) && (this.model !== null) && (this._class !== 'Viewport')) {
						arguments[i].setModel(this.model);
					}
					arguments[i].render();
					replacableDivParent.appendChild(arguments[i].getElement());
				} else {
					Throw(new InvalidParameterException("string|view|domelement", i));
				}
			}
		...
		addEvent: function(selector, eventType, eventName) {
			var _self = this;
			this.element.querySelector(selector)[eventType] = function(evt) {
	...
	Class('Viewport::View', {
		...
		init: function(delegate) {
			...
			document.body.onload = function() {
				_self.callDelegate('didBecomeActive');
			};
